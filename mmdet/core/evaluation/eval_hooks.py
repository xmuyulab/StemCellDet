...
class DistEvalHook(EvalHook):
    ...
    def after_train_epoch(self, runner):
        ...
        #         gdth.append(res['gdth'])
        #         # feat.append(res['feat'])
        #     import torch
        #     pred = torch.cat(pred, dim=0)
        #     gdth = torch.cat(gdth, dim=0)
        #     # feat = torch.cat(feat, dim=0)
        #     self.logger.info("acc: {}, recall: holoclone: {}, meroclone: {}, paraclone: {}, precision: holoclone: {}, meroclone: {}, paraclone: {} ".format(
        #                         (pred == gdth).float().mean(),
        #                         (pred == gdth)[gdth == 0].float().mean(),
        #                         (pred == gdth)[gdth == 1].float().mean(),
        #                         (pred == gdth)[gdth == 2].float().mean(),
        #                         (pred == gdth)[pred == 0].float().mean(),
        #                         (pred == gdth)[pred == 1].float().mean(),
        #                         (pred == gdth)[pred == 2].float().mean()
        #                      )
        #     )
        #     torch.save(results, osp.join(runner.work_dir, './feats.pth'))
