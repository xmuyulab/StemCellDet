...
class StandardRoIHeadOrganBase(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(StandardRoIHeadOrganBase, self).__init__(bbox_roi_extractor=bbox_roi_extractor,
                                              bbox_head=bbox_head,
        ...
        self.base_label_ids = torch.tensor(organ_base_label_ids, device=device)
        self.novel_label_ids = torch.tensor(organ_novel_label_ids, device=device)
        self.novel_index = F.pad(torch.bincount(self.novel_label_ids),
        ...
        # self.text_features_for_classes = nn.Parameter(torch.randn(3, 512), requires_grad=True)
        # self.project_for_image_clip = nn.Parameter(torch.randn(512, 512), requires_grad=True)
        # self.project_for_text_clip = nn.Parameter(torch.randn(512, 512), requires_grad=True)

    # def get_text_features(self):
    #     text_features_for_classes = torch.cat([self.clip_model.encode_text(clip.tokenize(desc).to(self.device)).detach() for desc in ORGAN_DESC], dim=0).float()
    #     text_features_for_classes = self.project_for_text_clip(text_features_for_classes)
    #     text_features_for_classes = F.normalize(text_features_for_classes, p=2, dim=-1)
    #     return text_features_for_classes
    ...
    def clip_image_forward(self, img_metas, bboxes, num_proposals_per_img, flag=False):
        ...
        for img_id, bbox in enumerate(bboxes):
            ...
            bbox = torch.dstack([torch.floor(bbox_raw[:, 0] - 0.001), torch.floor(bbox_raw[:, 1] - 0.001),
                                 torch.ceil(bbox_raw[:, 2] + 0.001), torch.ceil(bbox_raw[:, 3] + 0.001)]).squeeze(0)
