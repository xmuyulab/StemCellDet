...
from .multihead_attn import Multihead_Attention
...
class StandardRoIHeadOrgan(StandardRoIHeadOrganBase):
    ...
    def __init__(self,
        ...
        self.attn = Multihead_Attention(hidden_dim=512, num_heads=4)
    ...
    def _bbox_forward_train(self, x, img, img_no_normalize, sampling_results, proposals_pre_computed, gt_bboxes,
        ...
        text_embedding_part = torch.cat([self.text_features_for_classes[self.base_label_ids], bg_class_embedding], dim=0)
        text_embedding_part = text_embedding_part.repeat(region_embeddings.shape[0],1,1)
        region_embeddings = F.normalize(region_embeddings, p=2, dim=1) + F.normalize(self.attn(region_embeddings.unsqueeze(dim=1),
                                                                                        text_embedding_part,
                                                                                        text_embedding_part)[0].mean(dim=1), p=2, dim=1)
        ...
        text_cls_loss = F.cross_entropy(cls_score_text / self.temperature, labels,
                                        weight=torch.Tensor([1.0, 3.0, 3.0, 1.0]).cuda(), reduction='mean')
    ...
    def get_single_clip_finetune_feature(self, img, bboxes):
        ...
        clip_image_features_single = clip_image_features_single @ (self.project_for_image_clip * alpha + \
                                             torch.eye(self.project_for_image_clip.shape[-1], device=self.device) * (1.0-alpha)).T
    ...
    def forward_for_clip_finetune(self, img_no_normalize, gt_bboxes, gt_labels):
        ...
        cls_score_text[:, self.novel_label_ids] = -1e11
        text_cls_loss = F.cross_entropy(cls_score_text / self.temperature, labels,
                                        weight=torch.Tensor([3.0, 3.0, 1.0]).cuda(), reduction='mean')
