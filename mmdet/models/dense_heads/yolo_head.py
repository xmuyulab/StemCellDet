...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        loss_cls = self.loss_cls(pred_label, target_label, weight=pos_mask) / (pos_mask.sum() + 1e-8)
        loss_conf = self.loss_conf(
            pred_conf, target_conf, weight=pos_and_neg_mask) / (pos_and_neg_mask.sum() + 1e-8)
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes,
        ...
        target_map[sampling_result.pos_inds, :4] = self.bbox_coder.encode(
            ...
            anchor_strides[sampling_result.pos_inds]).float()
