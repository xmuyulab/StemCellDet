...
class SSDHead(AnchorHead):
    ...
    def __init__(self,
        ...
        for i in range(len(in_channels)):
            ...
            cls_convs.append(
                nn.Conv2d(
                    in_channels[i],
                    num_anchors[i] * 512, # L10 change for multi
        ...
        # L10 add for multi
        # self.conv_cls = nn.Conv2d(self.feat_channels, 512, 3, padding=1)
        # load text feature
        import clip
        import time
        ...
        self.text_features_for_classes = torch.cat(
            [self.clip_model.encode_text(clip.tokenize(desc).to(device)).detach() for desc in ORGAN_DESC],
            dim=0).float()
    ...
    def forward(self, feats):
        ...
        for feat, reg_conv, cls_conv in zip(feats, self.reg_convs,
            ...
            cls_feat_embed = cls_conv(feat)
            n, c, h, w = cls_feat_embed.shape
            cls_feat_embed = cls_feat_embed.reshape(n, c//512, 512, h, w)  # nbchw
            # cls_feat_embed = F.normalize(cls_feat_embed, p=2, dim=2)  # nbchw
            bg_class_embedding = self.bg_embedding(cls_feat_embed[0].new_ones(1)).reshape(1, 512)
            ...
            text_embed = F.normalize(text_features, p=2, dim=1)  # tc
