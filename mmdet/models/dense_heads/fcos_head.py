...
class FCOSHead(AnchorFreeHead):
    ...
    def forward_single(self, x, scale, stride):
        ...
        cls_score = torch.einsum('nchw, tc -> nthw', (cls_score, text_embed))
        # cls_score = torch.einsum('nchw, tc -> nthw', (F.normalize(cls_score, 2, dim=2), text_embed))
        # cls_score = (cls_score / 0.07)
    ...
    def loss(self,
        ...
        # # L10 change for multi
        # loss_cls = F.cross_entropy(flatten_cls_scores / 0.07,
        #                            flatten_labels,
        #                            reduction='sum') / (num_pos + num_imgs)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, centerness, points in zip(
            ...
            # scores = (scores / 0.07).softmax(dim=-1)
            scores = cls_score.permute(1, 2, 0).reshape(
